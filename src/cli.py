...
class OnboardCommand:
    ...
    gold_dataflowspec_table: str = "gold_dataflowspec"
    bronze_dataflowspec_path: str = None
    silver_dataflowspec_path: str = None
    gold_dataflowspec_path: str = None
    ...
    def __post_init__(self):
        ...
        if self.onboard_layer.lower() not in ["bronze", "silver", "gold", "bronze_silver", "bronze_silver_gold"]:
            raise ValueError("onboard_layer must be one of bronze, silver, gold, bronze_silver, bronze_silver_gold")
        # if self.uc_enabled == "":
        ...
        if self.onboard_layer and self.onboard_layer.lower() == "bronze_silver":
            ...
            if not self.uc_enabled:
                if not self.silver_dataflowspec_path:
                    raise ValueError("silver_dataflowspec_path is required")
        elif self.onboard_layer.lower() == "gold":
            if not self.gold_dataflowspec_table:
                raise ValueError("gold_dataflowspec_table is required")
            if not self.uc_enabled:
                if not self.gold_dataflowspec_path:
                    raise ValueError("gold_dataflowspec_path is required")
...
class DeployCommand:
    ...
    def __post_init__(self):
        ...
        if self.layer in ["gold", "bronze_silver_gold"]:
            if not self.onboard_gold_group:
                raise ValueError("onboard_gold_group is required")
            if self.uc_enabled and not self.dataflowspec_gold_table:
                raise ValueError("dataflowspec_gold_table is required")
            if not self.uc_enabled and not self.dataflowspec_gold_path:
                raise ValueError("dataflowspec_gold_path is required")
...
class DLTMeta:
    ...
    def _get_onboarding_named_parameters(self, cmd: OnboardCommand):
        ...
        if cmd.onboard_layer == "bronze_silver_gold":
            ...
            named_parameters["gold_dataflowspec_table"] = cmd.gold_dataflowspec_table
            if not cmd.uc_enabled:
                ...
                named_parameters["gold_dataflowspec_path"] = cmd.gold_dataflowspec_path
        elif cmd.onboard_layer == "bronze_silver":
            named_parameters["bronze_dataflowspec_table"] = cmd.bronze_dataflowspec_table
            named_parameters["silver_dataflowspec_table"] = cmd.silver_dataflowspec_table
            if not cmd.uc_enabled:
                named_parameters["bronze_dataflowspec_path"] = cmd.bronze_dataflowspec_path
    ...
    def _create_dlt_meta_pipeline(self, cmd: DeployCommand):
        ...
        if cmd.layer in ["bronze", "silver", "gold", "bronze_silver", "bronze_silver_gold"]:
            if cmd.layer in ["bronze", "bronze_silver", "bronze_silver_gold"]:
            ...
            if cmd.layer in ["silver", "bronze_silver", "bronze_silver_gold"]:
            ...
            if cmd.layer in ["gold", "bronze_silver_gold"]:
                configuration["gold.group"] = cmd.onboard_gold_group
                if cmd.uc_catalog_name:
                    configuration["gold.dataflowspecTable"] = (
                        f"{cmd.uc_catalog_name}.{cmd.dlt_meta_gold_schema}.{cmd.dataflowspec_gold_table}"
                    )
                else:
                    configuration["gold.dataflowspecTable"] = (
                        f"{cmd.dlt_meta_gold_schema}.{cmd.dataflowspec_gold_table}"
                    )
        else:
            raise ValueError("layer must be one of bronze, silver, gold, bronze_silver, bronze_silver_gold")
        created = None
