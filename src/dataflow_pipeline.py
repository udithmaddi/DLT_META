...
class DataflowPipeline:
    ...
    def __initialize_dataflow_pipeline(
        ...
        if isinstance(dataflow_spec, BronzeDataflowSpec):
            ...
            self.schema_json = None
        elif isinstance(dataflow_spec, GoldDataflowSpec):
            self.schema_json = None
        ...
        if hasattr(self.dataflowSpec, 'sourceDetails') and self.dataflowSpec.sourceDetails and self.dataflowSpec.sourceDetails.get("snapshot_format", None):
    ...
    def is_create_view(self):
        ...
        if (hasattr(self.dataflowSpec, 'sourceDetails') and self.dataflowSpec.sourceDetails and self.dataflowSpec.sourceDetails.get("snapshot_format") == "delta"):
    ...
    def _get_table_comment(self, target_table, is_bronze=True):
        ...
        if isinstance(self.dataflowSpec, GoldDataflowSpec):
            layer_name = "gold"
        else:
    ...
    def write_gold(self):
        """Write Gold tables."""
        if self.dataflowSpec.dataQualityExpectations and False:
            self.write_layer_with_dqe()
        else:
        ...
        if gold_spec.sources:
            ...
            for source in sources_list:
                ...
                if not source_table_name:
                    logger.warning(f"Source table name not found in source definition: {source}")
                    continue
                ...
                # We need to capture the values in a closure for the dlt.view function
                # Using default arguments in the lambda/function definition is a common pattern for this
                def make_source_view(t_name=source_table_name, streaming=is_streaming, f_cond=filter_condition):
                    def source_view_func():
                        reader = self._create_dataframe_reader(is_streaming=streaming)
                        df = reader.table(t_name)
                        if f_cond:
                            df = df.where(f_cond)
                        return df
                    return source_view_func

                if "reference_name" in source:
                    view_name = source["reference_name"]
                    
                    # Check if view already exists to avoid redefining error
                    if view_name in DataflowPipeline._created_views:
                        logger.info(f"Skipping creation of source view {view_name} because it was already created.")
                        continue
                        
                    dlt.view(
                        make_source_view(),
                        name=view_name,
                        comment=f"Source view for {view_name} from {source_table_name}"
                    )
                    DataflowPipeline._created_views.add(view_name)

    def _create_gold_dlt_views(self):
        """Create intermediate DLT views from dlt_views spec."""
        gold_spec: GoldDataflowSpec = self.dataflowSpec
        if gold_spec.dltViews:
            dlt_views_list = json.loads(gold_spec.dltViews)
            for view in dlt_views_list:
                sql_condition = view.get('sql_condition')
                view_name = view.get('reference_name')
                
                if sql_condition and view_name:
                    # Capture sql_condition in closure
                    def make_dlt_view_func(sql=sql_condition):
                        def dlt_view_func():
                            return self.spark.sql(sql)
                        return dlt_view_func
                    
                    if view_name in DataflowPipeline._created_views:
                        logger.info(f"Skipping creation of intermediate view {view_name} because it was already created.")
                        continue

                    dlt.view(
                        make_dlt_view_func(),
                        name=view_name,
                        comment=f"Intermediate transformation view {view_name}"
                    )
                    DataflowPipeline._created_views.add(view_name)
    ...
    def read_gold(self) -> DataFrame:
        ...
        if gold_spec.dltViews:
            dlt_views_list = json.loads(gold_spec.dltViews)
            if dlt_views_list:
                final_view = dlt_views_list[-1]
                final_view_name = final_view.get('reference_name')
                
                if final_view_name:
                    if self._is_gold_streaming():
                        return dlt.read_stream(final_view_name)
                    else:
                        return dlt.read(final_view_name)
        
        # 2. Fallback: Try to read from the first source view
        if gold_spec.sources:
            sources_list = json.loads(gold_spec.sources)
            if sources_list:
                first_source = sources_list[0]
                reference_name = first_source.get('reference_name')
                if reference_name:
                    if self._is_gold_streaming():
                        return dlt.read_stream(reference_name)
                    else:
                        return dlt.read(reference_name)
    ...
    def _is_gold_streaming(self) -> bool:
        ...
        if not isinstance(self.dataflowSpec, GoldDataflowSpec):
            return True
        
        if not self.dataflowSpec.sources:
            return False

        sources_list = json.loads(self.dataflowSpec.sources)
        for source in sources_list:
            is_streaming = True
            if "is_streaming" in source:
                is_streaming_val = source["is_streaming"]
    ...
    def invoke_dlt_pipeline(spark,
        ...
        if "bronze" == layer.lower():
            ...
            DataflowPipeline._launch_dlt_flow(
                spark, "gold", dataflowspec_list
            )
        elif "bronze_silver_gold" == layer.lower():
            bronze_dataflowspec_list = DataflowSpecUtils.get_bronze_dataflow_spec(spark)
            DataflowPipeline._launch_dlt_flow(
                spark, "bronze", bronze_dataflowspec_list, bronze_custom_transform_func,
                bronze_next_snapshot_and_version
            ...
            DataflowPipeline._launch_dlt_flow(
                spark, "gold", gold_dataflowspec_list
            )
