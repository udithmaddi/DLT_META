...
from src.dataflow_spec import BronzeDataflowSpec, DataflowSpecUtils, GoldDataflowSpec, SilverDataflowSpec
...
class OnboardDataflowspec:
    ...
    def onboard_gold_dataflow_spec(self):
        ...
        # sources and dltViews are arrays of maps/structs in JSON
        # We read them as strings (json serialized) or complex types then serialize
        columns = StructType(
            [
                StructField("sources", ArrayType(MapType(StringType(), StringType()), True), True),
                StructField("dlt_views", ArrayType(MapType(StringType(), StringType()), True), True),
                StructField("target_table", StringType(), True),
    ...
    def __get_gold_dataflow_spec_dataframe(self, onboarding_df, env):
        ...
        for onboarding_row in onboarding_rows:
            try:
                ...
                # Add path validation if relevant for Gold (assumed similar to Silver)
                pass
                # mandatory_fields.append(f"gold_table_path_{env}")
                # self.__validate_mandatory_fields(onboarding_row, mandatory_fields)
            
            # Gold layer mandatory check for path if not UC enabled
            if not self.uc_enabled:
                if f"gold_table_path_{env}" not in onboarding_row:
                     raise Exception(f"gold_table_path_{env} not provided in onboarding_row={onboarding_row}")

            gold_data_flow_spec_id = onboarding_row["data_flow_id"]
            gold_data_flow_spec_group = onboarding_row["data_flow_group"]

            gold_target_details = {
                "database": onboarding_row[f"gold_database_{env}"],
                "table": onboarding_row["gold_table"],
            }
            gold_cl = (
                onboarding_row[f"gold_catalog_{env}"]
                if f"gold_catalog_{env}" in onboarding_row
                else None
            )
            if "gold_table_comment" in onboarding_row:
                gold_target_details["comment"] = onboarding_row["gold_table_comment"]
            if gold_cl:
                gold_target_details["catalog"] = gold_cl
            if not self.uc_enabled:
                gold_target_details["path"] = onboarding_row[f"gold_table_path_{env}"]
            ...
            dlt_sinks = None
            if "gold_sinks" in onboarding_row and onboarding_row["gold_sinks"]:
                dlt_sinks = self.get_sink_details(onboarding_row, "gold")
            
            gold_cluster_by = self.__get_cluster_by_properties(onboarding_row, gold_table_properties,
                                                                 "gold_cluster_by")

            data_quality_expectations = None
            gold_quarantine_target_details = None
            gold_quarantine_table_properties = None
            ...
            if f"gold_data_quality_expectations_json_{env}" in onboarding_row:
                ...
                gold_quarantine_target_details, gold_quarantine_table_properties = self.__get_quarantine_details(
                    env, "gold", onboarding_row
                )
            ...
            gold_row = (
                ...
                gold_table_properties,
                data_quality_expectations,
                gold_quarantine_target_details,
                gold_quarantine_table_properties,
                gold_cluster_by,
                dlt_sinks
            )
